...
export function generateMockMinuteBars(count: number, seed = 42): Bar[] {
  const rand = mulberry32(seed)
  const bars: Bar[] = []
  let price = 0.86 + rand() * 0.04
  const endMs = Date.now()
  const startSec = Math.floor((endMs - count * 60 * 1000) / 1000) as UTCTimestamp
...
export function generateBtcUsdDemoMinuteBars(count: number, seed = 42): Bar[] {
  const rand = mulberry32(seed)
  const bars: Bar[] = []
...
export function generateGoldSpotMinuteBars(count: number, seed = 42): Bar[] {
  const rand = mulberry32(seed)
  const bars: Bar[] = []
  let price = 2640 + rand() * 45
  const endMs = Date.now()
