...
import { FEATURE_GROUPS, isBillingCycle, PRICING, type AccountTier as PlanTier } from './planCatalog'

export type AccountTier = PlanTier
...
function cell(value: string): string {
  ...
  if (value === '—') return '<span class="sx-sub__no" aria-label="Not applicable">—</span>'
  return `<span class="sx-sub__val">${value}</span>`
}

function featureTable(
  title: string,
  rows: ReadonlyArray<{ label: string; free: string; mid: string; pro: string }>,
  open: boolean,
): string {
  const body = rows
    .map(
  ...
  shell.innerHTML = `
    ...
    <div class="sx-sub-page__body">
      ...
      <section class="sx-sub-compare" aria-label="Feature comparison">
        ${FEATURE_GROUPS.map((group, i) => featureTable(group.title, group.rows, i === 0)).join('')}
      </section>
    </div>
    </div>
  `
